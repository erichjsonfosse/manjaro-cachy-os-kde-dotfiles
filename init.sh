...
if [ -f "$UNATTENDED_FILE" ]; then
  ...
  echo "DOTFILES_GIT_NAME=\"$GIT_NAME\"" > "$TEMPORARY_CONFIG_FILE_NAME"
  echo "DOTFILES_GIT_EMAIL=\"$GIT_EMAIL_ADDRESS\"" >> "$TEMPORARY_CONFIG_FILE_NAME"
  if [ -n "${GIT_SIGNING_KEY:-}" ]; then
    echo "DOTFILES_GIT_SIGNING_KEY=\"$GIT_SIGNING_KEY\"" >> "$TEMPORARY_CONFIG_FILE_NAME"
  fi
