...
if [ "$OS_ID" = "manjaro" ]; then
  ...
  sudo pacman-mirrors --country Austria,Canada,Denmark,France,Germany,Greece,Italy,Japan,Netherlands,Sweden,Switzerland,United_Kingdom
  ...
  if command -v reflector &>/dev/null; then
    sudo reflector --country Austria,Canada,Denmark,France,Germany,Greece,Italy,Japan,Netherlands,Sweden,Switzerland,United_Kingdom --latest 15 --sort rate --save /etc/pacman.d/mirrorlist || true
  fi
  ...
  if [ -f "/etc/pacman.d/cachyos-mirrorlist" ]; then
    ...
    awk -v pat="$trusted_pattern" '/^Server/ { if ($0 ~ pat) print; next } { print }' /etc/pacman.d/cachyos-mirrorlist > /tmp/cachyos-mirrorlist.tmp && sudo mv /tmp/cachyos-mirrorlist.tmp /etc/pacman.d/cachyos-mirrorlist
  fi
...
if grep -q "^#MAKEFLAGS=" /etc/makepkg.conf; then
  sudo sed -i "s/^#MAKEFLAGS=.*/MAKEFLAGS=\"-j$cores_to_use\"/" /etc/makepkg.conf
elif grep -q "^MAKEFLAGS=" /etc/makepkg.conf; then
  sudo sed -i "s/^MAKEFLAGS=.*/MAKEFLAGS=\"-j$cores_to_use\"/" /etc/makepkg.conf
else
  echo "MAKEFLAGS=\"-j$cores_to_use\"" | sudo tee -a /etc/makepkg.conf >/dev/null
fi
...
if [ "$OS_ID" = "manjaro" ]; then
  ...
  waitForPacmanLock
  sudo pacman -Rns --noconfirm cachyos-zsh-config || true
  packages+=(
    "paru"
    "reflector"
  )
